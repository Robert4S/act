...
impl RT {
    fn run_actor(pid: Pid, rt: &'static Arc<Mutex<RT>>) {
        let the_world = rt.lock().unwrap().the_world.clone();
        rt.lock().unwrap().init_actor(pid);
        loop {
            let lock = the_world.read().unwrap();
            match rt.lock().unwrap().update_actor(pid) {
                None => break,
    ...
    fn run_gc(&mut self) {
        let the_world = self.the_world.clone();
        let lock = the_world.write().unwrap();
        ...
        let roots = static_roots.chain(mailbox_roots).collect();
